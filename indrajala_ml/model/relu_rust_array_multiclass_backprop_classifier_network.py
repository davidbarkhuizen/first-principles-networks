...
class ReLURustArrayMultiClassBackpropClassifierNetwork:
    ...
    wholly separate class, not a subclass swapping a layer_cls extension point.

    Hidden layers are built from ReLURustArrayLayer; the output layer stays a plain
    RustArrayLayer (sigmoid) - the same hidden-layer-only split
    ReLUVectorizedMultiClassBackpropClassifierNetwork uses. No extra constructor parameter,
    matching that class's own posture - ReLU has no tunable coefficient.
    """
    ...
    def __init__(self, layer_sizes: list[int], dimension: int, class_count: int) -> None:
        ...
        for size in layer_sizes:
            self.layers.append(ReLURustArrayLayer(size, previous_size))
            previous_size = size
    ...
    @classmethod
    def load(cls, path: str) -> "ReLURustArrayMultiClassBackpropClassifierNetwork":
