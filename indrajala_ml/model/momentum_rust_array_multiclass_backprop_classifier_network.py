...
class MomentumRustArrayMultiClassBackpropClassifierNetwork:
    ...
    def __init__(
        ...
        self.layers: list[MomentumRustArrayLayer] = []
        previous_size = dimension
        for size in layer_sizes:
            self.layers.append(MomentumRustArrayLayer(size, previous_size, momentum))
            previous_size = size

        self.output_layer = MomentumRustArrayLayer(class_count, previous_size, momentum)
    ...
    @classmethod
    def load(cls, path: str) -> "MomentumRustArrayMultiClassBackpropClassifierNetwork":
