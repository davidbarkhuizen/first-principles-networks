...
class CrossEntropyVectorizedMultiClassBackpropClassifierNetwork:
    """
    A cross-entropy-output-layer sibling of VectorizedMultiClassBackpropClassifierNetwork - see
    docs/proposals/binary-cross-entropy-array-layer.md's "design" section. Mirrors
    SoftmaxVectorizedMultiClassBackpropClassifierNetwork's own precedent for adding an
    array-based sibling: a wholly separate class duplicating the same external contract, not a
    subclass swapping a layer_cls extension point.

    Unlike softmax's jointly-normalized output (one shared normalization across the whole
    output vector), this class's output layer applies an independent per-node cross-entropy
    delta at each of class_count outputs - a one-vs-rest-with-cross-entropy-loss variant, not a
    jointly-trained multiclass distribution: predict_probabilities does not sum to 1.0, and
    classify_state still picks the argmax, the same convention
    VectorizedMultiClassBackpropClassifierNetwork's own plain-sigmoid output already uses.

    Hidden layers stay plain ArrayLayer (sigmoid); only the output layer is a
    CrossEntropyArrayLayer - the array-level analogue of BinaryCrossEntropyBackpropClassifierNetwork's
    own output_layer_cls-only override, applied class_count-wide instead of single-output. No
    extra constructor parameter beyond layer_sizes/dimension/class_count, matching softmax's own
    posture - cross-entropy has no tunable coefficient either.
    """
    ...
    def __init__(self, layer_sizes: list[int], dimension: int, class_count: int) -> None:
        ...
        self.output_layer = CrossEntropyArrayLayer(class_count, previous_size)
    ...
    @classmethod
    def load(cls, path: str) -> "CrossEntropyVectorizedMultiClassBackpropClassifierNetwork":
