...
class DropoutRustArrayMultiClassBackpropClassifierNetwork:
    ...
    Hidden layers are built from DropoutRustArrayLayer; the output layer stays a plain
    RustArrayLayer (sigmoid) - the same hidden-layer-only split
    ...
    learn/learn_batch bracket only the forward pass in a set_training_mode(True)/try/finally, the
    same bracket shape DropoutVectorizedMultiClassBackpropClassifierNetwork uses (see that
    class's own docstring for the full reasoning - it carries over unchanged here).
    """
    ...
    def __init__(
        ...
        self.hidden_layers: list[DropoutRustArrayLayer] = []
        self.layers: list[RustArrayLayer] = []
        previous_size = dimension
        for size in layer_sizes:
            layer = DropoutRustArrayLayer(size, previous_size, drop_probability)
    ...
    @classmethod
    def load(cls, path: str) -> "DropoutRustArrayMultiClassBackpropClassifierNetwork":
