...
class L2RustArrayMultiClassBackpropClassifierNetwork:
    ...
    VectorizedMultiClassBackpropClassifierNetwork, applied one level further to the L2-regularized
    line. A wholly separate class, not a subclass swapping a layer_cls extension point.
    ...
    def __init__(
        ...
        self.layers: list[L2RustArrayLayer] = []
        previous_size = dimension
        for size in layer_sizes:
            self.layers.append(L2RustArrayLayer(size, previous_size, l2_lambda))
            previous_size = size

        self.output_layer = L2RustArrayLayer(class_count, previous_size, l2_lambda)
    ...
    @classmethod
    def load(cls, path: str) -> "L2RustArrayMultiClassBackpropClassifierNetwork":
