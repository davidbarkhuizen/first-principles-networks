...
class SoftmaxVectorizedMultiClassBackpropClassifierNetwork:
    """
    A softmax-output-layer sibling of VectorizedMultiClassBackpropClassifierNetwork - see
    docs/design-docs/array-siblings/softmax-array-layer.md. Mirrors ReLUVectorizedMultiClassBackpropClassifierNetwork's own
    ...
    Hidden layers stay plain ArrayLayer (sigmoid); only the output layer is a SoftmaxArrayLayer -
    the array-level analogue of SoftmaxMultiClassBackpropClassifierNetwork's own
    output_layer_cls-only override. No extra constructor parameter beyond layer_sizes/dimension/
    class_count, unlike momentum/L2/Adam's own siblings - softmax has no tunable coefficient.
    """
    ...
    def __init__(self, layer_sizes: list[int], dimension: int, class_count: int) -> None:
        ...
        self.output_layer = SoftmaxArrayLayer(class_count, previous_size)
    ...
    @classmethod
    def load(cls, path: str) -> "SoftmaxVectorizedMultiClassBackpropClassifierNetwork":
