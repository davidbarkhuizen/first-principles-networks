...
class ReLUVectorizedMultiClassBackpropClassifierNetwork:
    ...
    Hidden layers are built from ReLUArrayLayer; the output layer stays a plain ArrayLayer
    (sigmoid) - the array-level analogue of ReLUBackpropClassifierNetwork's own
    hidden_layer_cls-only override, matching ReLUNode's hidden-layer-only convention. No extra
    constructor parameter beyond layer_sizes/dimension/class_count, unlike momentum/L2/Adam's own
    siblings - ReLU has no tunable coefficient.
    """
    ...
    def __init__(self, layer_sizes: list[int], dimension: int, class_count: int) -> None:
        ...
        for size in layer_sizes:
            self.layers.append(ReLUArrayLayer(size, previous_size))
            previous_size = size
    ...
    @classmethod
    def load(cls, path: str) -> "ReLUVectorizedMultiClassBackpropClassifierNetwork":
