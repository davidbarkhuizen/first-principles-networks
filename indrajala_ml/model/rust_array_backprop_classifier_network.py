...
class RustArrayBackpropClassifierNetwork:
    """
    ...
    the numpy benchmark first. Mirrors ArrayBackpropClassifierNetwork's external contract exactly
    (learn, learn_batch, predict_probability, classify_state, randomize/randomized,
    snapshot/restore, save/load) so it's a drop-in swap - only the array backend
    (`indrajala_ml_array.Array` via RustArrayLayer, not numpy via ArrayLayer) differs.
    """
    ...
    def __init__(
        ...
        self.output_layer = RustArrayLayer(1, previous_size)
    ...
    def randomize(self) -> None:
        # the same fan-in-aware scheme ArrayBackpropClassifierNetwork.randomize uses - limit =
        # 1/sqrt(fan_in) - but drawn from indrajala_ml_array.uniform, not np.random.uniform; see
        # rust-array-core.md's "the RNG exception" for why this can never be seed-reproducible
        # against the numpy-backed sibling's own draws.
