...
class CrossEntropyRustArrayMultiClassBackpropClassifierNetwork:
    """
    The Rust-matmul-backed counterpart to
    CrossEntropyVectorizedMultiClassBackpropClassifierNetwork - see
    docs/proposals/binary-cross-entropy-array-layer.md. Mirrors that class's external contract
    exactly against CrossEntropyRustArrayLayer instead of CrossEntropyArrayLayer - the same
    relationship SoftmaxRustArrayMultiClassBackpropClassifierNetwork has to
    SoftmaxVectorizedMultiClassBackpropClassifierNetwork. A wholly separate class, not a subclass
    swapping a layer_cls extension point.

    Hidden layers are built from plain RustArrayLayer (sigmoid); only the output layer is a
    CrossEntropyRustArrayLayer - the same split
    CrossEntropyVectorizedMultiClassBackpropClassifierNetwork uses. Unlike softmax's own output
    (jointly normalized), predict_probabilities does not sum to 1.0, and classify_state still
    picks the argmax. No extra constructor parameter, matching that class's own posture -
    cross-entropy has no tunable coefficient.
    """
    ...
    def __init__(self, layer_sizes: list[int], dimension: int, class_count: int) -> None:
        ...
        self.output_layer = CrossEntropyRustArrayLayer(class_count, previous_size)
    ...
    @classmethod
    def load(cls, path: str) -> "CrossEntropyRustArrayMultiClassBackpropClassifierNetwork":
