...
class AdamRustArrayMultiClassBackpropClassifierNetwork:
    ...
    VectorizedMultiClassBackpropClassifierNetwork, applied one level further to the Adam-optimized
    line. A wholly separate class, not a subclass swapping a layer_cls extension point - neither
    AdamVectorizedMultiClassBackpropClassifierNetwork nor RustArrayMultiClassBackpropClassifierNetwork
    has one to hook either.
    ...
    def __init__(
        ...
        self.layers: list[AdamRustArrayLayer] = []
        previous_size = dimension
        for size in layer_sizes:
            self.layers.append(AdamRustArrayLayer(size, previous_size, beta1, beta2, epsilon))
            previous_size = size

        self.output_layer = AdamRustArrayLayer(class_count, previous_size, beta1, beta2, epsilon)
