...
class MomentumVectorizedMultiClassBackpropClassifierNetwork:
    ...
    MomentumBackpropClassifierNetwork's per-node counterpart already takes - and both hidden
    layers and the output layer are built from MomentumArrayLayer, mirroring
    MomentumBackpropClassifierNetwork's own hidden_layer_cls == output_layer_cls choice.
    ...
    def __init__(
        ...
        self.layers: list[MomentumArrayLayer] = []
        previous_size = dimension
        for size in layer_sizes:
            self.layers.append(MomentumArrayLayer(size, previous_size, momentum))
            previous_size = size

        self.output_layer = MomentumArrayLayer(class_count, previous_size, momentum)
    ...
    def save(self, path: str) -> None:
        save_array_model_json(
            ...
            extra={"momentum": self.momentum},
        )

    @classmethod
    def load(cls, path: str) -> "MomentumVectorizedMultiClassBackpropClassifierNetwork":
