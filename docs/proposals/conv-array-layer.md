...
this codebase's docs. This is exactly the case [structure](../project/structure.md#possible-next-steps)
...
digits](../research/research-backprop-siblings.md#convolutional-layers-on-uci-digits)), and the obvious
...
([structure](../project/structure.md#possible-next-steps)'s own "possible next steps" entry). That per-run
...
constraints [convolutional layers](../features/convolutional-layers.md) already scoped for the per-node
...
[research and analysis](../research/research-backprop-siblings.md)) is a variant *layered onto* the array
...
indexing - both are explicitly out of [the numpy interface subset](../architecture/numpy-interface-subset.md#explicitly-not-required)'s
...
core](../architecture/rust-array-core.md) has no windowing/gather primitive of any kind today.
...
- `test_conv_array_layer.py`: `_extract_patches`/`forward`/`compute_hidden_delta`/
  ...
  digits](../research/research-backprop-siblings.md#convolutional-layers-on-uci-digits) measured, plus a
...
- **The real-MNIST-scale validation this null result explicitly left open**: once wall-clock is
  ...
  [structure](../project/structure.md#possible-next-steps) already named as the natural next step, at real
...
- **A genuinely new Rust primitive, larger in scope than any other sibling's** - a windowing/gather
  ...
  workplan](../features/dropout.md#measurement-plan) used for its stage 2.
...
- **Stacked conv layers, multi-channel input, pooling** - out of scope here exactly as they are
  for the per-node version ([structure](../project/structure.md#convolutional-layer)); not reopened.
