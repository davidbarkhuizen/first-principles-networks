...
- `cargo`/`rustc` and `maturin` (for building [the Rust array core](../architecture/rust-array-core.md),
...
"dependencies" below - and see [vectorized array-based classes](../architecture/vectorized-array-classes.md) for
...
`rust/indrajala_ml_array/` - see [the production cutover plan](../architecture/rust-production-cutover.md) for why
...
This does **not** include [the Rust array core](../architecture/rust-array-core.md)'s own tests
(`rust/indrajala_ml_array/tests/`, 618 tests) - those are pytest tests against the built extension,
