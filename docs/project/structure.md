...
[research and analysis](../research/research-and-analysis.md) rather than assumed to help:
...
- **`AdamBackpropClassifierNetwork`** sets both hooks the same way too - a per-parameter
  ...
  rather than a default. See [research and analysis](../research/research-adam-optimizer.md) for the full
  measurement.
...
under large-batch training - see [research and analysis](../research/research-adam-optimizer.md)); one
...
[research and analysis](../research/research-multiclass-and-loss.md#parallelizing-mnist-training). The switch to
...
analysis](../research/research-multiclass-and-loss.md#the-ensemblereal-mnist-investigation)).
...
[research and analysis](../research/research-backprop-siblings.md#convolutional-layers-on-uci-digits)). Stacked
...
[vectorized array-based classes](../architecture/vectorized-array-classes.md#measured-results) for the full
...
array backend. [A hand-built Rust core](../architecture/rust-array-core.md) (`rust/indrajala_ml_array/`, wrapped
via PyO3) implements the same operation subset ([the numpy interface
subset](../architecture/numpy-interface-subset.md)) this class needs, is parity-tested against real numpy across
...
path, per [the production cutover plan](../architecture/rust-production-cutover.md)'s now-completed phases 0-2:
...
and
[research and analysis](../research/research-rust-performance.md#simd-for-the-matvec-production-path-a-bigger-win-than-the-batch32-work-it-followed)
...
[research and analysis](../research/research-and-analysis.md) for the investigations behind each decision).
...
- **Dropout** - `DropoutBackpropClassifierNetwork` is built and correctness-tested (see
  [dropout](../features/dropout.md)'s stage 2), but its actual regularization effect on this codebase's own
  ...
  **Update:** an array-based dropout sibling is now done, every stage -
  ...
  [dropout](../features/dropout.md#the-measurement-gap---not-run-deliberately-not-silently-dropped)'s own
  ...
  [research and
  analysis](../research/research-backprop-siblings.md#an-array-based-dropout-sibling-another-reconfirmed-null-now-at-real-power)
...
analysis](../research/research-adam-optimizer.md#rmsprop-the-second-moment-term-alone-accounts-for-adams-batch-size-win-stage-7-of-the-adam-optimizer-workplan)
for the full measurement.
...
follow-up](../research/research-backprop-siblings.md#the-learning-rate-vs-batch-size-follow-up-the-confound-was-real-and-momentum-still-doesnt-help)).
...
analysis](../research/research-backprop-siblings.md#the-batch_size128-divergence-retested-with-warmup) for the
...
- **A real-MNIST-scale convolutional layers validation**, now that the UCI-digits-scale result
  ...
  [research and analysis](../research/research-multiclass-and-loss.md#the-ensemblereal-mnist-investigation)), a
  real wall-clock cost this UCI-digits result alone doesn't settle is worth spending. Not yet
  run. [An array-based convolutional layer](../proposals/conv-array-layer.md) is the proposed, not-started
...
- **Array-based (numpy/Rust-matmul-backed) counterparts for the remaining per-node-only
  ...
  cross-entropy sibling. **Update:** [an array-based L2
  sibling](../research/research-backprop-siblings.md#an-array-based-l2-sibling-the-reconfirmed-null-holds-at-far-higher-power)
  ...
  sibling](../research/research-backprop-siblings.md#an-array-based-momentum-sibling-unchanged-on-stronger-evidence)
  ...
  sibling](../research/research-backprop-siblings.md#an-array-based-relu-sibling-a-genuine-scale-dependent-finding)
  ...
  sibling](../research/research-multiclass-and-loss.md#an-array-based-softmax-sibling-the-retune-the-per-node-investigation-declined-to-spend-on)
  ...
  sibling](../research/research-backprop-siblings.md#an-array-based-dropout-sibling-another-reconfirmed-null-now-at-real-power)
  ...
  sweep](../features/dropout.md#the-measurement-gap---not-run-deliberately-not-silently-dropped) affordable,
  ...
  sibling](../design-docs/array-siblings/binary-cross-entropy-array-layer.md) is also done, every
  stage, both backends (`CrossEntropyArrayLayer`/`CrossEntropyArrayBackpropClassifierNetwork`/
  `CrossEntropyVectorizedMultiClassBackpropClassifierNetwork` and their Rust-matmul-backed
  counterparts, the Rust stage needing no new Rust primitive at all - its delta formula is
  algebraically identical to the existing `pa.layer_softmax_output_delta`) - unlike the
  reconfirmed-null siblings above, the deferred real-MNIST retune this workplan existed to afford
  came back a genuine, if modest, win: at `learning_rate=0.1` cross-entropy beats the ensemble's
  documented quadratic baseline by 0.37 points, every cross-entropy seed outperforming every
  baseline seed - a different, more favorable outcome than the toy-XOR "matches, doesn't beat"
  finding this workplan set out to check whether it transfers. See [an array-based binary
  cross-entropy
...
- **An audit of hand-rolled measurement scripts for reusable infrastructure** - every real-scale
  ...
  shaped by each measurement's own specifics. [Benchmarking infrastructure](../architecture/benchmarking.md) is
  ...
  [benchmarking](../architecture/benchmarking.md#methodology), while every actual wall-clock table in the research
  ...
  [benchmarking](../architecture/benchmarking.md#scope)'s own 2026-09-17 update for the full reasoning. The new
