...
array siblings of `AdamBackpropClassifierNetwork` (see [structure](structure.md#backprop-siblings)).
...
`AdamBackpropClassifierNetwork`'s own resolved posture (see [structure](structure.md#backprop-siblings)).
...
cutover plan](rust-production-cutover.md#0b-fuse-each-layer-operation-into-one-rust-call) already
