...
[the Rust production cutover plan](rust-production-cutover.md) for precedent) - to be updated with
...
`W`/`b`-only snapshots). Whether `AdamVectorizedMultiClassBackpropClassifierNetwork` needs its own
extended snapshot/restore covering `m`/`v`/`t`, or can reuse the base contract unchanged (matching
this stage's actual use case), is a design decision to resolve during implementation, not assumed
...
- `test_adam_array_layer.py`: `AdamArrayLayer.apply_accumulated_gradient` checked directly against
  `AdamBackpropNode.apply_accumulated_gradient` (via `make_adam_node_cls`) - same starting
  weights/gradients/hyperparameters, both paths run for several steps, `pytest.approx`-compared at
  each one (not just the end state) - the same "matches after every step, not just at the end"
  discipline `test_vectorized_multiclass_backprop_model.py`'s own `learn`/`learn_batch` tests use.
- `test_adam_vectorized_multiclass_backprop_model.py`: whole-network parity against
  `AdamBackpropClassifierNetwork` (if scope allows a single-output comparison) or a hand-derived
  fixture (mirroring `test_adam_backprop_model.py`'s own regression convention) if the multi-class
  vs. single-output architectural mismatch makes direct parity impractical - resolved during
...
- **snapshot/restore scope for `m`/`v`/`t`** - see "design" above; not resolved here.
...
2. `AdamArrayLayer` + `AdamVectorizedMultiClassBackpropClassifierNetwork` + the parity-check tests
   above - the actual capability, buildable and mergeable independent of any later measurement.
