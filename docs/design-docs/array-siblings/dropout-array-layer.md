...
[dropout](../../features/dropout.md)). Built and measured; see [research and
...
[Dropout's own per-node design](../../features/dropout.md#design-a-real-minimal-exception-to-purely-additive-not-glossed-over)
...
Both subtleties [the per-node design](../../features/dropout.md#design-a-real-minimal-exception-to-purely-additive-not-glossed-over)
...
core](../../architecture/rust-array-core.md), which had no RNG surface at all before this (`randomize()`'s own random
draws happen in Python, via `np.random.uniform`, before ever touching the Rust core - see [the
numpy interface subset](../../architecture/numpy-interface-subset.md)'s own `uniform random fill` row): `random.rs`'s
...
- `test_dropout_array_layer.py`: `forward`/`compute_hidden_delta` at `training=False` checked
  ...
  analogue of [the per-node design](../../features/dropout.md#correctness-validation)'s own inference-mode
...
- `test_dropout_vectorized_multiclass_backprop_model.py`: whole-network tests via the shared
  ...
  [dropout's own per-node suite](../../features/dropout.md#correctness-validation) added. Not symmetry-breaking:
