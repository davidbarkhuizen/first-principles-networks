...
array siblings of `ReLUBackpropClassifierNetwork` (see [structure](../../project/structure.md#backprop-siblings)).
Built and measured; see [research and
analysis](../../research/research-backprop-siblings.md#an-array-based-relu-sibling-a-genuine-scale-dependent-finding)
...
against [the numpy interface subset](../../architecture/numpy-interface-subset.md#explicitly-not-required)'s own
...
core](../../architecture/rust-array-core.md)'s scope, not assumed to already exist there.
...
- `test_relu_rust_array_layer.py`/`test_relu_rust_array_multiclass_backprop_model.py` +
  ...
  gets - [the Rust array core](../../architecture/rust-array-core.md)), then the same two model-level tiers against
